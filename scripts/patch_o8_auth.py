...
pattern = r'const\{data:\{session\},error:sessionError\}=await supabaseClient\.auth\.getSession\(\);.*?supabaseClient\.auth\.onAuthStateChange\(\(_event,session\)=>\{authSession=session(?:\|\|null)?\}\);'
...
if n != 1 and 'direct-access fallback' not in s:
...
if n2 != 1 and 'Supabase save session check' not in s:
